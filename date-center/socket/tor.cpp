...
// Packet switch socket related methods -----------------------------------------
...
int PacketSwitch::isMasterOnline() {
    if ( pfds[MASTER].fd == pkFd )
        return 1;
    return 0;
}
    ...
    while (1) {
        ...
        for ( in = 0; in < MAXPKFD; in++ ) {
            if ( pfds[in].revents & POLLIN ) {
                ...
                if ( len <= 0 ) {
                    resetPfd(in);
                    cout << "port" << in << " lost connect" << endl;
                }
                ...
                switch(packetType) {
                    ...
                    case HELLO_ACK:
                        ...
                        break;
                    case DISCONNECT:
                        rcvDISCONNECT(frame.packet.disconnectPacket);
                        break;
                    case RELAY:
...
// Packet transfer related methods ----------------------------------------------
...
void PacketSwitch::sendDISCONNECT () {
    Packet disconnectPk = composeDISCONNECT(switchNum);
...
void PacketSwitch::rcvDISCONNECT (DISCONNECTPacket disconnectPk) {
    if ( disconnectPk.switchNum == 0 ) {
        fds[MASTER][0] = -1;
        fds[MASTER][1] = -1;
...
void PacketSwitch::resetPfd (int i) {
    pfds[i].fd = -1;
    pfds[i].events = 0;
    pfds[i].revents = 0;
}
